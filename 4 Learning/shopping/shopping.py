    ...
    with open(filename, 'r') as file:
        ...
        for row in reader:
            features = [
                ...
                float(row[8]), float(row[9]), convert_month(row[10]),
                int(row[11]), int(row[12]), int(row[13]), int(row[14]),
                convert_visitor_type(row[15]), convert_weekend(row[16])
            ]
...
def evaluate(labels, predictions):
    ...
    true_positive = sum(
        1 for true_label, prediction in zip(labels, predictions) if true_label == 1 and prediction == 1)
    true_negative = sum(
        1 for true_label, prediction in zip(labels, predictions) if true_label == 0 and prediction == 0)
